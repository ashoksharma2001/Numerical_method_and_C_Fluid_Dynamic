...
psi = zeros(n,m);
omega = zeros(n,m);
psi(n,:) = exp(xi(n)) * sin(theta);% Write the free-stream boundary
%% Set realx params, tol, extra vaiables
r_psi = 1.8; r_omega= 0.9;
...
while (error> delta)
    ...
    for i =2: n-1
        for j=2: m-1
            psi(i,j) = (1-r_psi)*psi(i,j) ...
                + (r_psi/4)*(psi(i+1, j) + psi(i-1, j) +psi(i,j+1) + psi(i,j-1) + fac(i)*omega(i,j)); 
        end
    end
    error_psi = max(abs(psi(:)-psi_old(:)));

    omega(1,:) = (1/(2*h^2)) * (psi(3,:) - 8 * psi(2, :)); % second order boundary condition
    for i=2:n-1
        for j=3:m-1
             f(i,j)= (psi(i+1,j)-psi(i-1,j))*(omega(i,j+1)-omega(i,j-1))...
               -(psi(i,j+1)-psi(i,j-1))*(omega(i+1,j)-omega(i-1,j));
            omega(i,j) = (1-r_omega)*omega(i,j) + (r_omega/4)*(omega(i+1, j) + omega(i-1,j) + omega(i,j+1) + omega(i,j-1) + (Re/8) * f(i,j));
        end
    end
